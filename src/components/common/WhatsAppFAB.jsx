...
export default function WhatsAppFAB() {
  ...
  return (
    ...
    >
      ...
      <div
        className={`mr-3 transition-all duration-300 transform origin-right ${
      ...
      >
        <a
          ...
          className="flex items-center gap-2.5 rounded-2xl bg-white/95 backdrop-blur-md px-3.5 py-2 text-slate-800 shadow-xl shadow-slate-900/10 border border-slate-200/80 hover:border-emerald-300 transition"
        >
          ...
          {showNotification && !isHovered && (
            <button
              ...
              className="ml-1 text-slate-400 hover:text-slate-600 text-xs font-bold leading-none p-0.5"
      ...
      <a
        ...
        className="relative flex h-14 w-14 sm:h-15 sm:w-15 items-center justify-center rounded-full bg-gradient-to-tr from-[#0A7039] via-[#0D8746] to-[#25D366] text-white shadow-xl shadow-emerald-600/35 hover:shadow-2xl hover:shadow-emerald-500/50 hover:scale-105 active:scale-95 transition-all duration-300 cursor-pointer"
      >
        {/* Pulsing Ripple Wave */}
        ...
        <span className="absolute top-0 right-0 flex h-4 w-4 items-center justify-center">
          <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-emerald-400 opacity-75" />
          <span className="relative inline-flex rounded-full h-3 w-3 bg-emerald-300 border-2 border-white" />
        </span>

        {/* Crisp WhatsApp SVG Icon */}
        <svg
          viewBox="0 0 32 32"
          className="h-7 w-7 sm:h-8 sm:w-8 fill-white drop-shadow-sm transition-transform group-hover:scale-110"
          aria-hidden="true"
        >
